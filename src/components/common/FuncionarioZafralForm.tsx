...
const schema = z
  .object({
    ...
    tareaId: z.string().min(1, 'Seleccioná una tarea'),
    tipoZafral: z.string().min(1, 'Selección requerida'),
...
interface FuncionarioZafralFormProps {
  initialValues?: FuncionarioZafral
  dependencias: Dependencia[]
  tareas: Tarea[]
  onSubmit: (values: CreateFuncionarioZafralInput) => Promise<void> | void
  onCancel: () => void
  submitting?: boolean
}
...
export function FuncionarioZafralForm({
  initialValues,
  dependencias,
  tareas,
  onSubmit,
  ...
  const {
    ...
    defaultValues: initialValues
      ? {
          ...
          tareaId: initialValues.tareaId ?? '',
          tipoZafral: initialValues.tipoZafral ?? '',
  ...
  const tareasFiltradas = useMemo(
    () =>
      dependenciaSeleccionada
        ? tareas.filter((item) => item.dependenciaId === dependenciaSeleccionada)
        : tareas,
    [tareas, dependenciaSeleccionada],
  )
  ...
  return (
    <>
      <form className={styles.form} onSubmit={submit} aria-label="Formulario de funcionario zafral">
        ...
        <section className={styles.section}>
          ...
          <div className={styles.row3}>
            ...
            <Controller
              ...
              name="tareaId"
              render={({ field }) => (
                <Field label="Tarea" validationMessage={errors.tareaId?.message} required>
                  <Dropdown
                    selectedOptions={field.value ? [field.value] : []}
                    value={tareasFiltradas.find((t) => t.id === field.value)?.nombre ?? tareas.find((t) => t.id === field.value)?.nombre ?? ''}
                    onOptionSelect={(_, data) => field.onChange(data.optionValue ?? '')}
                  >
          ...
          <div className={styles.row3}>
            <Controller
              ...
              name="tipoZafral"
              render={({ field }) => (
                <Field label="Tipo de zafral" validationMessage={errors.tipoZafral?.message} required>
                  <Dropdown
                    selectedOptions={[field.value]}
                    value={field.value}
                    onOptionSelect={(_, data) => field.onChange(data.optionValue)}
                  >
                    {(opciones?.tiposZafral ?? []).map((tipo) => (
              ...
              render={({ field }) => (
                <Field label="Estado" validationMessage={errors.estado?.message} required>
                  <Dropdown
                    ...
                    onOptionSelect={(_, data) => field.onChange(data.optionValue)}
                  >
                    {(opciones?.estadosZafral ?? []).map((estado) => (
